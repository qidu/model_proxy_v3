...
is not supported** — use a comma-separated string. See `./transforms-reference.md` for the
quick-reference cheat sheet and `../architecture/design_request_transform_hooks.md` for the design.
...
One **inline table per target** under a single `[passthrough]` section, keyed by
an arbitrary name. A request to `/passthrough/<path>` is forwarded verbatim to a
target whose `mode` matches the path, bypassing model routing,
composite/schedule, transforms, privacy filtering, and kompress. See
[Passthrough mode](./design_passthrough_mode.md) for the endpoint → mode map and
the schema gate.
...
Short field names are canonical here; the long aliases (`base_url`/`url`,
`api_key`/`key`, `upstream_mode`/`mode`) are also accepted. `[models.*]`
sections use the long names instead.

| Field | Example | Purpose |
|---|---|---|
| `base` | `"https://api.openai.com"` | Upstream origin plus optional path prefix. Joined to the client path by a plain `stripTrailingSlash(base) + '/' + path`, so the client path is never rewritten. Required. Aliases: `base_url`, `url`. |
| `mode` | `"openai-completions"` | Upstream protocol for this target: `openai-completions`, `anthropic-messages`, `openai-responses`, `gemini-generatecontent`, or `gemini-interactions`. Only endpoints mapping to this mode use the target. Required. Alias: `upstream_mode`. |
| `share` | `1` | Weight for weighted random selection among the targets sharing a `mode`. Default `1`. The weight only matters inside a pool: a `mode` with a single target always uses it, whatever its `share` says. |
| `timeout` | `60000` | Per-target upstream timeout in ms, overriding the global default. Optional. |
| `key` | `"sk-..."` | When set, sent upstream in place of the caller's key (overrides `[remote].auth_passthrough_with`). Optional. Alias: `api_key`. Covered by `[general] store_key_in_system` — the keychain account is `passthrough.<name>/<base>`. |

The `base` host is added to the shared SSRF allowlist alongside
...
is documented in [`../getting-started/README_DETAILS.md`](../getting-started/README_DETAILS.md).
