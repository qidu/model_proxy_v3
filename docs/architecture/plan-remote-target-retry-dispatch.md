...
- **The descriptor is self-contained; there is NO model-resolution step.** The client may
  ...
  matching `./auth-stats-protocol.md:79-80` literally. See Why the descriptor must be
  self-contained.
...
(`./auth-stats-protocol.md:71-73` lists only `target`/`mode`/`base`/`key`/`transforms`/
...
- Auth-response override uses the identical array/descriptor (see README
  "Auth dynamic-routing override" and `./auth-stats-protocol.md`).
...
- **Phase 1 treats `timeout` as a whole-request deadline**, which is what
  ...
  criterion. `./auth-stats-protocol.md:83-85` currently documents `timeout` as
...
- `src/utils/config-loader.ts` — **flat `[remote]` migration (breaking)**: rewrite the
  ...
  (`:3388-3389`).
- `README.md` + `./auth-stats-protocol.md` + `../reference/configuration-reference.md` — already
...
`./auth-stats-protocol.md` was updated recently and is now the operator-facing contract.
...
- Each rung's record POST carries **that rung's `otac`**; the `one_time_auth_code` header is
  ...
  (`./auth-stats-protocol.md:155-162`). Mechanically this is the closure variable
...
`./auth-stats-protocol.md:110-119` ("Non-2xx responses are recorded too… lets the collector
...
- **Doc/code conflict to resolve**: `./auth-stats-protocol.md:63-86` and
...
- **OTAC threading**: each rung must record with **its own** entry's `otac`, not the failed
  target's, or the admission↔usage linkage corrupts (`./auth-stats-protocol.md:155-162`).
