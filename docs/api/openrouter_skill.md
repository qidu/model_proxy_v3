...
## Free Models
- Extract free model_id from openrouter endpoints
```bash
# Free Models DO NOT store your data
curl -s https://openrouter.ai/api/v1/endpoints/zdr | jq | grep "model_id" | grep ":free"

# Free Models
curl -s "https://openrouter.ai/api/v1/models" | jq | grep "id" | grep ":free"
curl -s "https://openrouter.ai/api/v1/models" | jq | grep -v "description" | grep -A10 "id.*\:free\|supported_parameters" | grep "free\|tool[s\|\_choice]"
